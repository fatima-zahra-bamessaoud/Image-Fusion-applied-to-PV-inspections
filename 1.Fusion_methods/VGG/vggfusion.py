import numpy as np
from sporco.signal import tikhonov_filter  # Updated import

import torch
from torchvision.models.vgg import vgg19
from torchvision.models import VGG19_Weights  # Updated import for weights
...
def fuse(vis, ir, model=None):
    ...
    if model is None:
        model = vgg19(weights=VGG19_Weights.IMAGENET1K_V1)  # Updated to use keyword argument
    model.cpu().eval()  # Use CPU instead of GPU
    relus = [2, 7, 12, 21]
    unit_relus = [1, 2, 4, 8]
    
    vis_in = torch.from_numpy(c3(vis_high)).cpu()  # Use CPU
    ir_in = torch.from_numpy(c3(ir_high)).cpu()  # Use CPU
